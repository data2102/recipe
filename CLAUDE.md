...
- **닮은 레시피는 판정하지 말고 짝만 보여준다** (`lib/similar.ts`). 이름은
  글자 2-gram, 재료는 `raw_name` 집합으로 각각 자카드를 재고 **둘 중 큰
  쪽**을 쓴다 — 이름이 전혀 다른 같은 요리(감바스/버터갈릭새우)는 재료로만,
  재료를 아직 안 붙인 것은 이름으로만 잡힌다. 겹친 재료가 3개 미만이면
  비율이 커도 근거로 안 센다 (재료 두 개짜리는 뭐에나 포함된다).
  **`/similar` 의 문턱(30%)은 임시다** — 실제로 몇 쌍이 걸리는지 보고
  정하려고 낮게 잡아둔 것이고, 저장 확인 화면에 붙일 값은 그걸 보고 정한다.
  자동 병합은 만들지 마라: 같은 이름이어도 다른 레시피일 수 있다
