...
- **흔한 재료는 겹침을 부풀린다.** 대파·양파는 어디에나 들어가서, 개수만
  세면 김치찌개와 제육볶음이 세 가지 겹쳐 한 묶음이 됐다. 그래서 재료마다
  IDF 로 무게를 낮추고 (`weighted`), **넷 넘게 겹쳐야** 근거로 센다
  (`MIN_SHARED`). 화면에는 가중치가 아니라 **겹친 개수**를 적는다 —
  사람이 읽는 근거는 "몇 가지가 같다" 다
