...
- **"별로였어요" 는 지운다** (`markBad`). 예전에는 `status='BAD'` 로 숨기기만
  했는데 쓰는 사람이 지우기로 정했다. 되돌릴 수 없어서 시트가 한 번 더 묻는다.
  `shopping_list_recipe` 만 CASCADE 가 없으니 (지난 주 기록이 요리 하나
  지웠다고 사라지면 안 된다) **손으로 먼저 떼야 삭제가 된다**
