 ...
 * 네이티브 앱은 서버 액션을 못 쓰고 `app/api/ingest` 를 타는데, 하는
 * 일이 액션 안에 있으면 저쪽에 한 벌을 더 쓰게 된다. 그러면 "원본을
 * 파싱보다 먼저 보관한다" 같은 **순서 규칙이 두 군데**가 되고, 한쪽만
...
import { PARSER_VERSION, ParseError, parse } from "./parse";
import { assetKeys, recordAsset, recordParsed, save, saveTitleOnly } from "./store";
...
/**
 * 올린 것 한 벌. 화면이 무엇이든(폼·멀티파트) 여기까지 오면 값이다.
 */
export type Upload = {
  /** 파일 이름 — 오류 문장에 쓴다 ("○○ 이 너무 커요") */
  name: string;
  type: string;
  bytes: Buffer;
};
...
export async function ingest(input: {
  ...
  const files = input.images.filter((f) => f.bytes.length > 0);
  ...
  try {
    for (const file of files) {
      if (file.bytes.length > MAX_BYTES) {
        return {
          ok: false,
          message: `${file.name} 이 너무 커요 (${Math.round(file.bytes.length / 1e6)}MB).`,
          hint: "8MB 아래로 줄여주세요.",
        };
      }
...
export async function commit(draft: Draft): Promise<number> {
  ...
  /*
    화면을 터는 건 여기 일이 아니다 — 그건 Next 의 캐시 무효화이고
    화면이 있을 때만 뜻이 있다. 부른 쪽이 한다: 서버 액션은 세
    화면을 털고, API 는 털 화면이 없다.
  */
  return recipeId;
}
