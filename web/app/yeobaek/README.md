...
4. **모바일 타입 스케일.** 기본값이 데스크톱 기준이다 (본문 15 / 캡션 13 /
   각주 12). 폰에서 한 손으로 보는 화면에서는 캡션이 안 읽힌다 — 이 앱은
   `globals.css` 에서 16 / 14 / 13 으로 올려 쓴다. 원본에 폭 기준
   (`@media (max-width: 480px)`) 스케일이 있어야 화면마다 덮어쓰지 않는다.
