...
--  7. 그날의 메모  — "저녁 약속 있어요"
--
--  식단을 짜다 보면 **못 먹는 날**이 생긴다. 약속이 미리 잡혀 있거나
--  갑자기 생기거나. 그런 날에 메뉴를 담아두면 지나간 뒤에 "만들었어요?"
--  를 물어보게 되고, 안 만든 게 만든 것으로 남거나 물음만 쌓인다.
--
--  요일이 아니라 **날짜**다 (shopping_list_recipe.day_of_week 와 다르다).
--  약속은 주에 속한 게 아니라 그 하루에 있는 일이고, 주가 넘어가도
--  "9월 16일에 회식이었다" 는 그대로 남아야 한다.
--
--  하루에 한 줄이다. 저녁·점심을 나누지 않는다 — v1 은 저녁 한 끼를
--  정하는 앱이고, 칸을 늘리면 안 채운 칸이 먼저 생긴다.
-- ---------------------------------------------------------------------

CREATE TABLE day_note (
    on_date         DATE PRIMARY KEY,

    -- 사용자가 적은 그대로. 빈 문자열은 안 남긴다 — 지우면 행을 지운다
    -- (있는데 비어 있는 메모와 없는 메모를 구별할 이유가 없다).
    note            TEXT NOT NULL,
